...
2. Pick or type the **Namespace**; a token is for one namespace. The list under the field holds the namespaces you used before with this role (marked with a clock), then the ones the role allows. Typing filters it, so a role with hundreds of namespaces stays one field. The field is filled in with the last namespace you used, else the first one the role allows. The helper line says what the role accepts: a short list, a count, `Any namespace`, or the labels of `allowed_kubernetes_namespace_selector`.
