...
import './step/the_tokens_are_issued_for_the_namespaces.dart';
import './step/i_choose_the_namespace.dart';
import './step/i_add_the_namespace.dart';
import './step/i_set_the_cluster_address_with_the_ca.dart';
import './step/i_copy_the_kubeconfig.dart';
import './step/the_copied_kubeconfig_has_a_context_for_each_of_the_namespaces.dart';
import './step/i_revoke_the_tokens.dart';
import './step/i_do_not_see_the_text.dart';
import './step/i_type_in_the_namespace_field.dart';
...
void main() {
  group('''Kubernetes credentials''', () {
    ...
    testWidgets('''Only the chosen namespace gets a token''', (tester) async {
      await bddSetUp(tester);
      await theKubernetesAccessScreenIsOpen(tester);
      await iPickTheRole(tester, 'developer');
      await iRequestAKubernetesToken(tester);
      await theTokensAreIssuedForTheNamespaces(tester, 'team-a');
    });
    testWidgets('''Several namespaces give one token each''', (tester) async {
      await bddSetUp(tester);
      await theKubernetesAccessScreenIsOpen(tester);
      await iPickTheRole(tester, 'developer');
      await iChooseTheNamespace(tester, 'team-b');
      await iRequestAKubernetesToken(tester);
      await theTokensAreIssuedForTheNamespaces(tester, 'team-a, team-b');
      await iSeeTheText(tester, 'Revoke all');
    });
    testWidgets('''A refused namespace does not stop the others''', (
      tester,
    ) async {
      await bddSetUp(tester);
      await theKubernetesAccessScreenIsOpen(tester);
      await iPickTheRole(tester, 'developer');
      await iAddTheNamespace(tester, 'default');
      await iRequestAKubernetesToken(tester);
      await theTokensAreIssuedForTheNamespaces(tester, 'team-a');
      await iSeeTheText(tester, 'default: Namespace default is not allowed.');
    });
    testWidgets('''The kubeconfig has one context per namespace''', (
      tester,
    ) async {
      await bddSetUp(tester);
      await theKubernetesAccessScreenIsOpen(tester);
      ...
      await theCopiedKubeconfigHasAContextForEachOfTheNamespaces(
        tester,
        'team-a, team-b',
      );
    });
    testWidgets('''Revoking removes every token from the screen''', (
      tester,
    ) async {
      await bddSetUp(tester);
      await theKubernetesAccessScreenIsOpen(tester);
    ...
    testWidgets('''The namespaces offered narrow down as I type''', (
      ...
      await theKubernetesAccessScreenIsOpen(tester);
      await iPickTheRole(tester, 'developer');
      await iTypeInTheNamespaceField(tester, 'b');
