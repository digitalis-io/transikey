...
- **Kubernetes**: `features/kubernetes` mirrors database (mounts from `sys/internal/ui/mounts`, `kubernetesMount` fallback, one card, generation counter). API server + CA path saved in `kubernetesTargets` keyed by mount. Token only in clipboard (via `ClipboardGuard`) or a `writePrivateFile` (0600) kubeconfig; `kubectlCommand` uses `KUBECONFIG=`, never argv
...
| k3s | `:6443` (API), CA via `make dev-k3s-ca` → `dev/k3s-ca.crt` | mount `kubernetes`: roles `developer` (Role, namespace `transikey-test`), `viewer` (ClusterRole, any namespace, cluster binding); tokens min `10m` TTL |
