...
- **Kubernetes**: `features/kubernetes` mirrors database (mounts from `sys/internal/ui/mounts`, `kubernetesMount` fallback, one card, generation counter). API server + CA path saved in `kubernetesTargets` keyed by mount; the CA file is read (`kubeCaReaderProvider`, absolute path) and embedded as `certificate-authority-data`. Several allowed namespaces show as chips. Token only in clipboard (via `ClipboardGuard`) or a `writePrivateFile` (0600) kubeconfig; `kubectlCommand` uses `KUBECONFIG=`, never argv
